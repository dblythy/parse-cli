...
(async () => {
  try {
    ...
    await run(
      "git clone --single-branch --branch dotenv https://github.com/dblythy/parse-server-example.git ."
    );
    ...
    if (library === "JavaScript + jQuery") {
      const location = require
        .resolve("./lib/utils.js")
        .replace("/utils.js", "");
      await run(
        `cat ${location}/samples/JavaScript/script.js > public/assets/js/script.js`
      );
      await run(
        `cat ${location}/samples/JavaScript/test.html > public/test.html`
      );
      if (appId) {
        await run(`sed -i '' 's/myAppId/${appId}/g' public/test.html`);
        await run(
          `sed -i '' 's/myAppId/${appId}/g' public/assets/js/script.js`
        );
      }
    } else if (library === "React") {
      // TODO -> Sample React Web App
    } else {
      // TODO -> Sample VueJS Web App
    }
    console.log(
      `Your Parse Server is ready to go!\n\nRun:\n${manager} start\n\n and visit http://localhost:1337/test to begin.`
    );
  } catch (e) {
    console.log({ e });
